...
public class LPPlatform {
    ...
    private static HashMap<String, String> encryptEncryptableFields(Boolean override, String fieldName, String fielddValue){        
        HashMap<String, String> hm = new HashMap<>();        
        StringBuilder newFieldValueBuilder = new StringBuilder();
        if (!fieldName.toUpperCase().contains("IN")){
            Object[] encStr = encryptString(fielddValue);
            if (override){
                newFieldValueBuilder.append(encStr[1].toString());
            }else{
                fieldName=fieldName+" in|";       
                newFieldValueBuilder.append(fielddValue).append("|").append(encStr[1]);
            }
        }else{
            SqlStatement sql = new SqlStatement();
            String separator = sql.inSeparator(fieldName);
            String[] valuesArr = newFieldValueBuilder.toString().split(separator);
